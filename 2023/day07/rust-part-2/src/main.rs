...
const CARD_ORDER: [u8; 14] = [
    b'J', b'2', b'3', b'4', b'5', b'6', b'7', b'8', b'9', b'T', b'J', b'Q', b'K', b'A'
];
...
impl Hand {
    fn hand_type(&self) -> HandType {
        ...
        let js = card_cnt.remove(&b'J').unwrap_or_default();
        if js == 5 {
            return FiveOfAKind
        }
        let max_cnt = card_cnt.clone().into_iter().max_by(|x, y| x.1.cmp(&y.1)).unwrap();
        card_cnt.remove(max_cnt.0);

        if max_cnt.1 + js > 2i64 {
            return if max_cnt.1 + js == 3i64 {
                return if card_cnt.iter().any(|(x, y)| *y == 2i64) {
                    FullHouse
                } else {
                    ThreeOfAKind
                }
            } else if max_cnt.1 + js == 4i64 {
                FourOfAKind
            } else {
                FiveOfAKind
            };
