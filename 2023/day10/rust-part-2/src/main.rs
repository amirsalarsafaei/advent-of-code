...
fn find_loop((i,j) : (i32, i32), (n,m) : (usize, usize), mv: Move, seen: &mut HashMap<(i32, i32), i32>,
                ...
                for mv in Move::iter() {
                    ...
                    if let Some(value) = res{
                        ...
                        for i in 0..n {
                            ...
                            for j in 0..m {
                                let ch = table.get(i).unwrap().get(j).unwrap();

                                if let Some(_tmp) = loop_set.get(&(i as i32, j as i32)) {
                                    if ch == &'L' || ch == &'J' || ch == &'|'{
                                        inside = !inside;
                                    }
                                } else if inside {
