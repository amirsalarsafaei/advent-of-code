...
fn main() {
    ...
    for i in 0..n {
        res = max(res,
                  solve(Move{i: i as i32, j: 0, dir: Right}, &table));
        res = max(res,
                  solve(Move{i: i as i32, j: m as i32 - 1, dir: Left}, &table));
    }

    for j in 0..m {
        res = max(res,
                  solve(Move { i: 0, j: j as i32, dir: Down }, &table));
        res = max(res,
                  solve(Move { i: n as i32 - 1, j: j as i32 - 1, dir: Up }, &table));
    }
    println!("{}", res);
}
fn solve(start_move: Move, table: &Vec<Vec<char>>) -> i32 {
    let mut seen: HashSet<Move> = HashSet::new();
    let (n, m) = (table.len(), table.first().unwrap().len());

    dfs(start_move, &table, &mut seen);
